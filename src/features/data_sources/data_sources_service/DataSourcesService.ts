...
import deentitifyDataSource from "./deentitifyDataSource.js";
...
class DataSourcesService {
	...
	public async getDataSources(pagingOptions: PagingOptions): Promise<Page<DataSource>> {
		return (await paginatedFindAndCount(this.dataSourcesRepository, pagingOptions)).map(
			deentitifyDataSource
		);
	}
	public async getDataSourceById(dataSourceId: string): Promise<DataSource> {
		try {
			return deentitifyDataSource(
				await this.dataSourcesRepository.findOneByOrFail({id: dataSourceId})
			);
		} catch (error) {
	...
	public async getDataSourceBySlug(dataSourceslug: string): Promise<DataSource> {
		try {
			return deentitifyDataSource(
				await this.dataSourcesRepository.findOneByOrFail({slug: dataSourceslug})
			);
		} catch (error) {
			if (error instanceof EntityNotFoundError) {
	...
	public async addDataSource(addDataSourcePayload: AddDataSourcePayload): Promise<DataSource> {
		...
		return deentitifyDataSource(savedDataSourceEntity);
	}
