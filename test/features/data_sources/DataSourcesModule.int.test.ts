...
import DataSourcesModule from "../../../src/features/data_sources/data_sources_module/DataSourcesModule.js";
...
describe("DataSourcesModule", () => {
	...
	beforeEach(async () => {
		...
		const appModule = await Test.createTestingModule({
			imports: [DataSourcesModule, AppOrmModule, AppConfigModule],
		}).compile();
	...
	describe("v1", () => {
		describe("Empty database", () => {
			...
			test("GET /data-sources/:id", async () => {
				const response = await app.inject({
					method: "GET",
					url: "/v1/data-sources/1",
				});
				expect(response.statusCode).toBe(400);
			});
			test("POST /data-sources", async () => {
			...
			test("GET /data-sources/:id with invalid UUID4", async () => {
				const response = await app.inject({
					method: "GET",
					url: "/v1/data-sources/1",
				});
			...
			test("GET /data-sources/:id with valid UUID4", async () => {
				const response = await app.inject({
					method: "GET",
					url: "/v1/data-sources/e8a7b311-367b-4105-a75d-929b930faafa",
				});
				expect(response.statusCode).toBe(404);
			});
		});

		describe("Database with one DataSource", () => {
			test("GET /data-sources", async () => {
				const addDataSourceRequestBody = {
					name: "test2",
					slug: "test2",
				...
				expect((({id, ...rest}) => rest)(responseJson.items[0])).toEqual(addDataSourceRequestBody);
			});
		});
	});
});
